...
WHERE hukum.visible = 1 AND hukum.id = $id");
    ...
    public function get_braile($id)
    {
        return $this->db->query("SELECT * FROM `file_braile` WHERE file_id_hukum = " . $id);
    }

    public function get_audio($id)
    {
        return $this->db->query("SELECT * FROM `file_alih_suara` WHERE file_id_hukum = " . $id);
    }
    ...
    // upload braille
        $uploadDir = './asset/file_braile/';
        if (!is_dir($uploadDir)) mkdir($uploadDir, 0777, true);

        if (!empty($_FILES['braile']['name'])) {

            $fileTmp = $_FILES['braile']['tmp_name'];
            $originalName = $_FILES['braile']['name'];
            $ext = pathinfo($originalName, PATHINFO_EXTENSION);

            $newFileName = 'braille_No_' . $this->input->post('no') . '_Tahun_' . $this->input->post('tahun') . '_' . uniqid() . '.' . $ext;
            $destPath = $uploadDir . $newFileName;

            if (move_uploaded_file($fileTmp, $destPath)) {

                $this->db->insert('file_braile', [
                    'file_path' => '/asset/file_braile/' . $newFileName,
                    'file_id_hukum' => $id
                ]);
            }
        }
        ...
        // upload alih suara
        $uploadDir = './asset/file_alih_suara/';
        if (!is_dir($uploadDir)) mkdir($uploadDir, 0777, true);

        if (!empty($_FILES['alih_suara']['name'])) {

            $fileTmp = $_FILES['alih_suara']['tmp_name'];
            $originalName = $_FILES['alih_suara']['name'];
            $ext = pathinfo($originalName, PATHINFO_EXTENSION);

            $newFileName = 'audio_No_' . $this->input->post('no') . '_Tahun_' . $this->input->post('tahun') . '_' . uniqid() . '.' . $ext;
            $destPath = $uploadDir . $newFileName;

            if (move_uploaded_file($fileTmp, $destPath)) {

                $this->db->insert('file_alih_suara', [
                    'file_path' => '/asset/file_alih_suara/' . $newFileName,
                    'file_id_hukum' => $id
                ]);
            }
        }

        // upload naskah kuno versi teks saja
        $teks = trim($this->input->post('teks_naskah_kuno'));

        if (!empty($teks)) {
            $this->db->insert('file_naskah_kuno', [
                'teks' => $teks,
                'file_id_hukum' => $id
            ]);
        }
    ...
    function get_list_peraturan($postData = null)
    {
        ...
        foreach ($records as $record) {
            ...
            ##split format judul untuk beberapa dokumen
            $jenis = strtoupper($record->jenis_nama);
            ...
            if (in_array($jenis, $jenis_tentang_only)) {
                
                // Hanya tampil tentang
                $judul_tampil = toUpperCase($record->nama);
                
            } elseif ($jenis == 'PROGRAM PEMBENTUKAN PERATURAN') {
                
                // Jenis + No + Tentang (tanpa tahun)
                $judul_tampil = toUpperCase(
                    $record->jenis_nama . ' NO ' . $record->no . ' TENTANG ' . $record->nama
                );
                
            } else {
                
                // Default (format lama)
                $judul_tampil = toUpperCase($record->judul);
            }
    ...
    function get_list_bahasa($postData = null)
    {
        ...
        $baseFilter = "LENGTH(TRIM(v_hukum.path_file_inggris)) > 0 AND UPPER(TRIM(jenis_nama)) != 'DOKUMEN LANGKA'";
        ...
        ## Total records with filtering version new
        $this->db->select('count(*) as allcount');
        $this->db->where($baseFilter);

        if (count($search_arr) > 0) {
            $this->db->where(implode(" AND ", $search_arr));
        }

        $records = $this->db->get('v_hukum')->result();
        $totalRecordwithFilter = $records[0]->allcount;

        ## Fetch Data version new
        $this->db->select('*');
        $this->db->where($baseFilter);
    ...
    function get_list_disabilitas($postData = null)
    {
        ...
        ## Search 
        $search_arr = array();
        $searchQuery = " 1=1 AND ( LENGTH(TRIM(v_hukum2.path_file_alih_suara)) > 0 
                       OR LENGTH(TRIM(v_hukum2.path_file_braile)) > 0 )";
        if ($searchValue != '') {
            $search_arr[] = " (tahun like '%" . $searchValue . "%' or 
                kategori like '%" . $searchValue . "%' or 
                nama_inggris like'%" . $searchValue . "%' ) ";
        ...
        ## Total number of records without filtering
        $this->db->select('count(*) as allcount');
        $records = $this->db->get('v_hukum2')->result();
        $totalRecords = $records[0]->allcount;
        ...
        foreach ($records as $record) {
            ...
            if (!empty($record->path_file_alih_suara)) {
                $audio = base_url($record->path_file_alih_suara);
                $fileStatus = "
                ...
                $fileStatus = "<span class='badge bg-secondary'>NO AUDIO</span>";
            }

            if (!empty($record->path_file_braile)) {
                $braille = base_url($record->path_file_braile);
                $fileview = "
                    <a target='_blank' href='$braille' class='btn btn-success btn-sm'>
                        <i class='fa fa-universal-access'></i> BRAILLE
                    </a>
                ";
            } else {
                $fileview = "<span class='badge bg-secondary'>NO BRAILLE</span>";
            }



            $data[] = array(
                "nama" =>"<p style=' text-align: justify;font-family: Arial, sans-serif;font-size: 11px;'>".toUpperCase($record->nama)."</p>",
                "no" => toUpperCase($record->no),
                "status_nama" => toUpperCase($record->status_nama),
                "jenis_nama" => toUpperCase($record->jenis_nama),
                "tanggal_ditetapkan" => toUpperCase(tgl_indo($record->tanggal_ditetapkan)),
